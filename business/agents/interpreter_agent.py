...
def _infer_region_filter(q: str) -> Optional[Dict[str, str]]:
    """Визначає region_filter з тексту. Київ НЕ входить до Київської області — для області виключаємо місто."""
    if any(x in q for x in ("по всім джерелам", "по всіх джерелах", "всі джерела", "всі регіони", "по всіх регіонах")):
        return None
    # «Київська область» / «Київської області» БЕЗ «та», «Києві» — тільки область, виключаємо м. Київ
    oblast_only = (
        ("київська область" in q or "київської області" in q or "київська обл" in q)
        and "київ та" not in q
        and "в києві та" not in q
        and "києві та" not in q
    )
    if oblast_only:
        return {"region": "Київська", "exclude_city": "Київ"}
    # «Київ та область», «в Києві та області», «Києву» — місто АБО область
    if any(x in q for x in ("києву", "києвській області", "київ", "київська", "київ та область")):
