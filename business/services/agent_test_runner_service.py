...
class AgentTestRunnerService:
    ...
    def _llm_interpret_response_success(
        ...
        try:
            ...
            prompt = f"""Ти — експерт з оцінки відповідей помічника, що працює з даними ProZorro та OLX.
...
Питання: чи була відповідь помічника успішною? Тобто чи помічник реально використав дані або коректно пояснив результат (наприклад, якщо даних справді немає — це успіх; якщо в БД дані є, а помічник написав «не вдалося знайти даних» — це невдача).

Поверни ТІЛЬКИ один JSON-об'єкт без markdown та без іншого тексту:
{{"success": true або false, "reason": "одне коротке речення українською"}}"""
    ...
    def _generate_test_cases_via_llm(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            lc = LangChainAgentService(self.settings)
            llm = lc._get_llm()
            prompt = """Ти — тест-агент. Згенеруй рівно 5 тест-кейсів для перевірки LLM-помічника, що працює з даними ProZorro (аукціони) та OLX (оголошення).
