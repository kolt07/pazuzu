...
class MistaSettlementImportService:
    ...
    def import_one(self, raw_doc: Dict[str, Any], *, dry_run: bool = False) -> Dict[str, Any]:
        ...
        aliases = parsed.get("search_aliases") or []
        ...
        if existing:
            ...
            merged_aliases = sorted(
                set(existing.get("search_aliases") or []) | set(meta.get("search_aliases") or [])
            )
            if merged_aliases:
                updates["search_aliases"] = merged_aliases
            new_name = name
            if display_name_quality(new_name) > display_name_quality(existing.get("name")):
                updates["name"] = new_name
                updates["name_normalized"] = normalize_settlement_key(new_name)
            self.cities_repo.update_by_id(existing["_id"], {"$set": updates})
            return {"status": "updated", "city_id": str(existing["_id"]), "name": updates.get("name", existing.get("name"))}
    ...
    def run_import(self, *, limit: int = 0, dry_run: bool = False) -> Dict[str, Any]:
        docs = self.raw_repo.find_parsed(limit=limit)
