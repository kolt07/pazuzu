...
class PriceAnalyticsService:
    ...
    def _compute_indicators(self) -> int:
        """Обчислює індикатори ціни (квартилі по містах та областях) за останній місяць."""
        cutoff = datetime.now(timezone.utc) - timedelta(days=INDICATOR_DAYS)
        count = 0

        # 1. Індикатори по містах
        pipeline_city = [
            {"$match": {"source_updated_at": {"$gte": cutoff}, "status": "активне"}},
        ...
        for row in self.unified_repo.collection.aggregate(pipeline_city):
            ...
            for metric, vals_key in [
                ...
                self.repo.upsert_indicator(city, region, metric, q1, q2, q3, q4, len(vals))
                count += 1

        # 2. Індикатори по областях (fallback, коли по місту < 3 оголошень)
        pipeline_region = [
            {"$match": {"source_updated_at": {"$gte": cutoff}, "status": "активне"}},
            {"$unwind": {"path": "$addresses", "preserveNullAndEmptyArrays": False}},
        ...
        for row in self.unified_repo.collection.aggregate(pipeline_region):
            region = row["_id"] if isinstance(row["_id"], str) else ""
            if not region:
                continue
            for metric, vals_key in [
                ("price_uah", "price_uah_vals"),
                ...
                if len(vals) < 3:
                    continue
                q1, q2, q3, q4 = _quartiles(vals)
                self.repo.upsert_region_indicator(region, metric, q1, q2, q3, q4, len(vals))
                count += 1

        return count

    def _compute_aggregates(self) -> int:
        """Обчислює агреговані метрики за днями, тижнями, місяцями."""
        pipeline = [
            {"$match": {"status": "активне"}},
            {"$unwind": {"path": "$addresses", "preserveNullAndEmptyArrays": False}},
            {
                "$group": {
                    "_id": {
    ...
    def get_price_indicator(
        ...
        Якщо по місту < 3 оголошень — використовується метрика по області.
        Типовий діапазон: [q1 - 1.5*IQR, q3 + 1.5*IQR] (межквартильний розмах).
        """
        ...
        ind = None
        if city:
            ind = self.repo.get_indicator(city, metric, region)
            if ind and ind.get("count", 0) >= self.MIN_INDICATOR_COUNT:
                pass
            else:
                ind = None
        if not ind and region:
            ind = self.repo.get_region_indicator(region, metric)
        if not ind:
            return None
    ...
    def get_price_indicators_for_items(
        ...
        items мають містити: city, region, price_uah, price_per_m2_uah, price_per_ha_uah.
        Returns: composite_id -> "вигідна"|"середня"|"дорога" (для основної метрики)
        """
        result = {}
        for item in items:
            city = item.get("city")
            ...
            if metric and value:
                ind = self.get_price_indicator(value, city, metric, region)
                if ind:
                    composite_id = f"{item.get('source', '')}:{item.get('source_id', '')}"
                    result[composite_id] = ind
        return result
