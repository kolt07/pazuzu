...
- **Дії**: `scripts/ops/restore_mongodb_collections.py` — читає JSONL/meta/manifest, insert_many пакетами, індекси з `.meta.json`; без `--drop` дублікати `_id` пропускаються.

## 2026-09-21 — Дамп колекцій MongoDB

- **Запит**: скрипт, який підключається до Mongo застосунку і вивантажує вміст колекцій у окремий каталог поза git.
- **Дії**: `scripts/ops/dump_mongodb_collections.py` (Settings + MongoDBConnection, JSONL Extended JSON, manifest); каталог `data/mongo_dumps/` додано в `.gitignore`.

## 2026-09-18 — Дослідження: таймаут LLM без звалища UUID

- **Проблема**: дослідження падало з `Timed out waiting for tasks:` і списком усіх Celery `task_id` (обрізалось на 2000 символів).
- **Причина**: `wait_for_all` після дедлайну кидав усі ідентифікатори; UI дослідження показував raw `error`.
- **Дії**: короткий підсумок `готово N/M (queued=…, running=…)`; при pause/disable черги — одразу зупиняти очікування; дослідження й source-load продовжують з уже завершеними LLM-задачами, обмеження потрапляє в довідку.

## 2026-09-18 — Excel у бот: відсутній openpyxl

- **Проблема**: `send_research_via_bot` падав з `ImportError: Для форматування Excel потрібно встановити openpyxl`.
- **Причина**: `generate_excel_in_memory` імпортує `openpyxl`, але пакет не був у `requirements.txt` (pandas його не тягне).
- **Дії**: додано `openpyxl>=3.1.0` до `requirements.txt`.

## 2026-09-08 — Закриті угоди ProZorro: вул. Олександра Довженка, 3 (Київ)

- **Запит**: знайти в джерелі ProZorro всі закриті угоди продажу й оренди приміщень за адресою м. Київ, вул. Олександра Довженка, 3 (за весь час); цікавлять переможці.
- **Дії**: QueryBuilder по `unified_listings` / `prozorro_auctions` (у локальній БД 0 записів); Search API ProZorro.Sale (`POST /api/search/procedures`, регіон Київ + `full_text_search` «Довженка» + status `complete`) і GET `/procedures/{id}`; переможці з `awards[].buyers`. Результат: 52 завершені оренди `regulationsPropertyLease-english` (орендодавець АТ «Поліграфкнига», ЄДРПОУ 02470135), продажів приміщень немає.

## 2026-08-24 — Дослідження: у UI лише постфільтр

- **Запит**: прибрати сирі «знайдено / оброблено», лишити цифри після відбору.
- **Дії**: KPI — вибірка / OLX / ProZorro (`matched*`); рядок статусу без found/new/processed; Telegram без «Знайдено в джерелах».

## 2026-08-24 — Дослідження: кнопки історії в нічному режимі

- **Проблема**: «Повторити» в бічній панелі зливалось з карткою (той самий фон і слабка рамка).
- **Дії**: «Повторити» — заливка accent; «Видалити» / «Скасувати» — контрастний фон і рамка окремо для dark theme.

## 2026-08-24 — Дослідження: час обробки та витрати Vast

- **Запит**: у UI і в Telegram-повідомленні про завершення показувати час обробки та витрати Vast (баланс на початку обробки − баланс після завершення).
- **Дії**: знімок `credit`/`balance` через `/users/current/` на старті Phase 2 і при `done`; поля `duration_sec`, `vast_cost_usd`, `vast_balance_*` у `market_research_runs`; блок «Час / Vast» у UI; рядок у bot message.

## 2026-08-20 — Дослідження: адмін бачить усі запуски

- **Запит**: адміністратор має бачити всі дослідження всіх користувачів, з ім'ям автора і можливістю керування (переглядати/скасовувати/видаляти/повторювати).
- **Дії**: `list_all` в репозиторії та сервісі; `_owned_run` з `is_admin` обходом; `user_name` (nickname) в `_public_run`; API `_get_user_ctx` повертає `is_admin`; UI — badge з нікнеймом користувача у списку історії.

## 2026-08-20 — Дослідження ProZorro: фільтри не звужували вибірку

- **Проблема**: у логах ~1680 hits / ~1050 raw при фільтрах (12 областей, оренда, площа 4500–6000) — виглядало як «тягне все».
- **Причини**: (1) Search API не фільтрує за площею — постфільтр дослідження ігнорував area/property_type/geo; (2) region values з короткими назвами; (3) статус «активне» не йшов у Search.
- **Дії**: канонічні «X область» / «Київ» для Search; status active_*; full_text «нежитлове» для комерції; early reject по площі при details; постфільтр — area/price/property_type/geo; лог `filters=...`.

## 2026-08-20 — Дослідження: авто-сповіщення в бот і Excel

- **Запит**: коли дослідження завершилось, повідомити користувача в боті й одразу надіслати таблицю результатів.
- **Дії**: після `STATUS_DONE` сервіс шле bot message + `Оголошення_дослідження_*.xlsx`; дубль блокується полем `bot_notified_at`.

## 2026-08-20 — Geocoding: retry на SSL/мережеві збої

- **Проблема**: `SSLEOFError` при запиті до Google Geocoding — адреса без координат; у лог потрапляв URL з API key.
- **Дії**: 3 спроби з backoff 1/2/4 с; у логах — без key і query string.



- **Запит**: HTTP list-спроби завжди 403; одразу браузер, перший метод прибрати.
- **Дії**: `_fetch_olx_list_via_browser`; HTTP list/fallback/backoff видалено; `fetch_page` лише для detail.



- **Проблема**: на сторінці оголошення 530 000 $ відображались як 530 000 ₴ (~11 836 $).
- **Причина**: UI завжди ставив ₴ до `price_value`; `price_metrics` теж брав суму як гривню.
- **Дії**: показ за `search_data.currency`; конвертація в грн перед метриками; `price_currency` в unified.



- **Запит**: зручніша верхня частина запуску; вкладка застосованих фільтрів; список на всю висоту; кнопка «Видалити».
- **Дії**: форма в правій колонці кроками 1–2; «Нове» в шапці списку; вкладка «Фільтри»; історія stretch; «Видалити» в списку і в тулбарі.



- **Причина**: URL зі списку з `?search_reason=` не збігались з канонічним `source_id`; LLM часто ще не писав картку, постфільтр бачив порожньо.
- **Дії**: канонічні URL у ключах і `$in`; після LLM — sync навіть для «інше»; якщо картки немає — чернетка з raw (заголовок/ціна).

## 2026-08-19 — Дослідження: Excel оголошень і PDF-довідка в бот

- **Запит**: завантажити оголошення і довідку; у довідці — висновки без DA-термінів; PDF; файли через Telegram-бот.
- **Дії**: висновки `conclusions_from_report`; PDF (fpdf2 + системний шрифт); Excel як у пошуку; кнопки «Оголошення в бот» / «Довідка PDF в бот».

## 2026-08-19 — Форма нового дослідження за кнопкою

- **Запит**: блок запуску сховати, показувати по «Нове дослідження».
- **Дії**: форма згорнута за замовчуванням; кнопка відкриває, «Закрити» / успішний запуск ховає; «Скасувати» активного прогону лишається в тулбарі.

## 2026-08-19 — Дослідження: лише ProZorro + UI

- **Проблема**: без фільтра джерела у результаті лишався ProZorro; екран досліджень був нечитабельним.
- **Причина**: постфільтр ганяв вибірку через пошук (гео + status=активне + $in URL) і відсікав OLX.
- **Дії**: постфільтр лише по знайдених ключах (без гео/ціни/дефолтного статусу); джерела — окремі перемикачі; вкладки «Оголошення / Довідка», KPI, історія збоку.

## 2026-08-19 — Постфільтр дослідження: хибна суперечність джерел

- **Проблема**: після вибірки OLX+ProZorro постфільтр логував «Суперечливі умови по полю Джерело» і відкочувався до сирих ключів без відборів.
- **Причина**: перевірка збирала всі `source=eq` у дереві; OR `(source=olx AND id∈…) ∨ (source=prozorro AND id∈…)` виглядав як AND двох джерел.
- **Дії**: суперечність лише коли в одній AND-групі перетин джерел порожній.

## 2026-08-19 — OLX 403 через некоректний query

- **Проблема**: робочий URL має `currency=USD` без `search[order]`; згенерований — `currency=USD%3A` і `search[order]=created_at:desc` → 403.
- **Дії**: `currency` пишемо першим як `USD` (без `:`); дослідження ринку збирає list URL з `sort_newest=False`.

## 2026-08-19 — Дослідження ринку: OLX вибірка 0 при наявності оголошень

- **Проблема**: «Готово: 0 у вибірці» при «нових 3» і реальних результатах на OLX.
- **Причини**:
  1. Дослідження брало лише `loaded_urls` (нові/змінені raw). Вже відомі оголошення скіпались — `found=0`.
  2. Ціна в `$` йшла в OLX як грн без `search[currency]=USD`.
  3. Скрапер завжди додавав дефолт площі (50 м² / 15 соток), навіть якщо користувач площу не задавав.
  4. НП «Київ» мапився на область `/ko/` замість міста `/kiev/`.
- **Дії**: `include_unchanged_urls` для research; валюта `currency=USD` (не `search[currency]` — той дає 404); `apply_default_area_filters=False`; місто з OLX-slug (Київ) як region.
...
## 2026-08-19 — +30% очікування pull моделі на орендованому GPU

- **Запит**: збільшити час очікування скачування LLM-моделі арендованим GPU-сервером на 30%.
- **Дії**: `ready_timeout_sec` 300→390 с (дефолт і fallback оркестратора). Нормалізація більше не обрізає значення до 300 с (стеля 1800 с). У `config.example.yaml` приклад 1200→1560 с.
...
- Зроблено: `PUT /api/report-templates/{id}` + `ReportTemplateRepository.update` / `ReportTemplateService.update_template`; у списку шаблонів кнопка «Редагувати» (лише не системні); конструктор зберігає через PUT при редагуванні; поле назви з `for`/`autocomplete`, вертикальний layout `name-row`, явне зняття readonly/disabled у JS.

## 2026-05-26 — ОНМ: фільтр галюцинацій LLM (зайві об'єкти з OLX)

- Запит: оголошення «Продам або обміняю на автомобіль є 5 ділянок» отримало 11 ОНМ, яких не було в тексті.
- Причини: у текст для LLM потрапляли адреси з `detail.llm.addresses`; не було пост-перевірки результату; порожній список ОНМ не очищав `real_estate_object_refs`.
- Виправлено: `utils/real_estate_objects_validator.py` (grounding по тексту, ліміт за «N ділянок», дедуп); прибрано llm.addresses з промпту ОНМ; фільтр лише для LLM-виводу (ProZorro items без змін); `update_real_estate_object_refs` приймає `[]`; посилено `real_estate_objects_parsing` у prompts.yaml; тести `tests/test_real_estate_objects_validator.py`.
...
- **Дії**: У `VllmRuntimeOrchestrator.ensure_runtime_ready()` додано обов'язкову перевірку ownership coordination lease перед **кожною** новою спробою оренди (`_coord_owner_is_self()` + re-acquire через `try_acquire/wait_for_shared_runtime_or_acquire`). Це закриває race: після `teardown` lease звільнявся, але поточний процес міг піти на наступний `create_instance` без повторного lock.
- **Тести**: У `scripts/test_vllm_runtime_orchestrator.py` додано unit-тест на сценарій retry після teardown: перевіряє, що друга спроба не створює новий контракт, якщо lock вже перехопив інший процес, а використовує shared endpoint.
...
- **Дії**:
  - **config/config_export_service.py**: Виправлено некоректні імпорти `config.config...` на актуальні `config...`:
    - `config.config.config_loader` → `config.config_loader`
    - `config.config.settings` → `config.settings`
  - Через ці імпорти модуль `config_export_service` не завантажувався, що ламало ендпоїнти mini app (`/api/admin/export/*`, `/api/admin/import/config`) з HTTP 500.
  - Проведено smoke-перевірку імпорту та виклику `build_config_zip()` — модуль завантажується коректно, ZIP формується.
...
## 2026-04-22 — OLX: виправлення масштабу площі "сотки/сотих" + backfill даних
- Запит: усунути помилку, коли площа землі на кшталт `25 сотих` некоректно зсувається по масштабу (до 250/2500 соток), виправити існуючі записи та не допустити повторення.
- Додано `utils/land_area_utils.py` з єдиною логікою:
  - витяг площі із форм `сотка/соток/сотих` (включно з `25сотих`);
  - дедуплікація повторів площі між заголовком і описом;
  - детектор типових scale-mismatch (`x10`, `x100`, `÷10`, `÷100`).
- У `business/services/olx_llm_extractor_service.py` інтегровано нову нормалізацію площі із соток для online-обробки обмінів.
- У `business/services/llm_service.py` уніфіковано конвертацію одиниць `сот*` у гектари (`*0.01`) для всіх провайдерів, прибрано помилкове множення, що могло роздувати площу.
- У `utils/listing_regex_extractor.py` розширено патерни для `сотих/сотки/...` та уніфіковано конвертацію.
- Додано міграцію `scripts/migrations/045_olx_land_area_sotok_backfill.py`, яка:
  - сканує `olx_listings` (title + description),
  - виправляє `detail.llm.land_area_sqm/land_area_ha`,
  - оновлює пов'язані `unified_listings` та цінові метрики,
  - запускає повний перерахунок аналітики через `PriceAnalyticsService.rebuild_all()`.

## 2026-04-22 — Гео-фільтри: канонічний список областей і нормалізація топонімів
- Запит: прибрати "сміття" з поля `Область` в інтерфейсі та гарантувати, що `Чернігівська обл./область/о. Чернігівська` шукаються як один топонім.
- Додано `utils/ukraine_regions.py` з канонічним переліком регіонів України в міжнародно визнаних межах (включно з `АР Крим`, `м. Київ`, `м. Севастополь`) та уніфікованою нормалізацією/regex-патернами.
...
## 2026-08-19 — Керування дослідженнями ринку
- Запит: зручне керування дослідженнями — відмінити, повторити, видалити, подивитись результат.
- `MarketResearchRepository.delete_run` — видалення завершених досліджень (не активних).
- `MarketResearchService.delete` / `.repeat` — видалення та повторний запуск з тими ж фільтрами.
- API: `DELETE /api/market-research/{id}`, `POST /api/market-research/{id}/repeat`.
- UI: кнопки дій в історії (📋 результат, 🔄 повторити, 🗑 видалити, ✕ скасувати), статуси кольорові.

## 2026-08-19 — OLX: обмеження пагінації за кількістю оголошень
- **Проблема**: при market research (і звичайному скрапінгу) OLX повертає 403 для неіснуючих сторінок пагінації. При 108 оголошеннях (≈3 сторінки) скрапер намагався зайти на сторінку 8+, отримував 403, чекав backoff — зайві затримки та помилки.
- **Виправлення**: при першому запиті парситься "Ми знайшли N оголошень" (без "понад"); обчислюється `estimated_pages = ceil(N / 50)` і обмежується `max_pages`. Для "понад 1 000" обмеження не застосовується. Виправлено в обох `_process_category` та `_process_category_raw_only`.
