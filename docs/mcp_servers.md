...
py -m mcp_servers.export_mcp_server
py -m mcp_servers.geocoding_mcp_server
py -m mcp_servers.data_update_mcp_server
```
...
- `olx_listings` - колекція оголошень OLX (нежитлова нерухомість, земельні ділянки)
...
**Параметри**:
- `collection_name` (string) - назва колекції (`prozorro_auctions`, `llm_cache` або `olx_listings`)
...
```json
{
  "mcpServers": {
    ...
    "export-mcp": {
      "command": "py",
      "args": ["-m", "mcp_servers.export_mcp_server"],
      "env": {
        "PYTHONPATH": "."
      }
    },
    "geocoding-mcp": {
      "command": "py",
      "args": ["-m", "mcp_servers.geocoding_mcp_server"],
      "env": {
        "PYTHONPATH": "."
      }
    },
    "data-update-mcp": {
      "command": "py",
      "args": ["-m", "mcp_servers.data_update_mcp_server"],
      "env": {
        "PYTHONPATH": "."
      }
    }
...
MCP сервер `export-mcp` отримує від агента перелік ідентифікаторів оголошень/аукціонів та, опціонально, структуру полів; викликає механізм отримання з БД та збереження у файл; повертає агенту **посилання на розташування файлу** (url). Агент використовує його: (1) коли користувач явно просить відповідь у вигляді Excel/файлу; (2) автоматично, коли кількість оголошень у відповіді перевищує 10.

### Інструменти (Tools)

#### `export_listings_to_file`

Експортує оголошення або аукціони за списком ідентифікаторів у файл у тимчасовій папці; повертає url.

**Параметри**:
- `ids` (list) — список ідентифікаторів (для prozorro_auctions — auction_id або _id; для olx_listings — url або _id)
- `collection` (string) — `prozorro_auctions` | `olx_listings`
- `format` (string, опціонально) — xlsx | csv | json (за замовчуванням xlsx)
- `columns` (list, опціонально) — поля у dot-notation (наприклад auction_data.value.amount)
- `column_headers` (dict, опціонально) — мапінг ключ → назва колонки
- `filename_prefix` (string, опціонально) — префікс імені файлу

**Повертає**: success, url (шлях до файлу), filename, size, rows_count; або success=False та error.

#### `get_export_collections`

Повертає список колекцій, доступних для експорту (prozorro_auctions, olx_listings).

---

## Geocoding MCP Server

MCP сервер `geocoding-mcp` надає геокодування адрес і топонімів (назви ЖК, вулиці, міста, ТРЦ тощо) через Google Geocoding API з кешуванням. Результати зберігаються в колекції `geocode_cache`; в оголошеннях можна зберігати `query_hash` замість текстового представлення.

### Інструмент

#### `geocode_address`

Перетворює текстовий опис адреси або топоніма на координати. Спочатку перевіряється кеш; при відсутності — запит до Google API.

**Параметри**:
- `address_or_place` (string) — адреса, назва ЖК, вулиця, місто тощо
- `region` (string, опціонально) — код регіону для зміщення результатів (за замовчуванням `ua`)

**Повертає**:
- `query_hash` — хеш запиту (для збереження в оголошенні)
- `query_text` — оригінальний текст запиту
- `results` — список місць: `latitude`, `longitude`, `formatted_address`, `place_id`, `types`, `location_type`
- `from_cache` — чи результат взято з кешу

---

## Data Update MCP Server

MCP сервер `data-update-mcp` надає інструменти для ініціювання оновлення даних у базі. Агент викликає їх, коли користувач просить оновити або перезавантажити дані OLX.

### Інструменти (Tools)

#### `trigger_olx_update`

Запускає оновлення оголошень OLX у БД (нежитлова нерухомість, земельні ділянки). Скрапер обходить сторінки пошуку OLX, завантажує деталі, обробляє через LLM та геокодування, зберігає в колекцію `olx_listings`.

**Параметри**:
- `days` (number, опціонально) — якщо 1 або 7: збір обмежується періодом (остання доба або тиждень). Якщо не вказано — збір за кількістю сторінок (за замовчуванням категорії).

**Повертає**: success, message, total_listings, total_detail_fetches, by_category; або success=False та error.

#### `get_data_update_sources`

Повертає список джерел, для яких можна ініціювати оновлення (olx; prozorro — лише інформація про те, що оновлення через пайплайн/Telegram).
...
1) Звіт з джерела даних (dataSource):
```json
{
  "format": "xlsx",
...
2) Експорт за ідентифікаторами оголошень у БД (уніфікований механізм збереження у файл; файл створюється в тимчасовій папці, повертається `url` або base64):
```json
{
  "format": "xlsx",
  "collection": "prozorro_auctions",
...
```
- `collection`: `prozorro_auctions` | `olx_listings`
- `columns`, `column_headers`, `filename_prefix` — опціональні

Або з повним запитом (джерело даних):
```json
{
  "format": "xlsx",
  "template": "auction_summary",
