# -*- coding: utf-8 -*-
"""
Міграція 022: Приведення price_per_m2 та price_per_ha до правил заповнення.
...
- Земельна ділянка з нерухомістю (обидві площі > 0):
  заповнюємо обидва поля
- Чисто нерухомість (building_area_sqm > 0, land_area_ha відсутня):
  price_per_m2 = заповнена, price_per_ha = NULL

Міграція повторно витягує площі з сирих документів (з виправленою логікою для OLX:
не використовувати area_m2 як fallback для building при наявності land_area_ha),
потім перераховує метрики.

Запуск: py scripts/migrations/022_price_metrics_land_rules.py
"""
...
def _extract_area_from_raw_doc(doc: Dict[str, Any], source: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    Логіка збігається з UnifiedListingsService._extract_area_info (з виправленням для OLX).
    """
...
def run_migration() -> bool:
    ...
    print("=" * 60)
    print("Міграція 022: Приведення price_per_m2 та price_per_ha до правил")
    print("=" * 60)
    ...
    try:
        ...
        for doc in cursor:
            ...
            # Витягуємо площі з сирого документа (з виправленою логікою)
            building_area_sqm = None
            land_area_ha = None
            raw_doc = None
            if source == "olx":
            ...
            if raw_doc:
                ...
                # Якщо сирого документа немає — використовуємо збережені значення
        ...
        print(f"Помилка міграції 022: {e}")
        import traceback
        traceback.print_exc()
        return False
    finally:
